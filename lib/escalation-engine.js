...
class EscalationEngine {
   ...
   */
  async checkStuckTasks(thresholdOverride, maxPriority) {
    const rules = await this.db.getMany(
      `SELECT * FROM escalation_rules WHERE trigger_condition = 'timeout' AND enabled = TRUE`
    );

    const stuckTasks = [];
    const priorityFilter = maxPriority ? `AND t.priority <= ${parseInt(maxPriority, 10)}` : '';
    ...
    for (const rule of rules) {
      ...
      const tasks = await this.db.getMany(
        `SELECT t.* FROM tasks t
         WHERE t.status = 'in_progress'
           ...
           ${catFilter} ${agentFilter} ${priorityFilter}
      ...
      for (const task of tasks) {
        ...
        const escCount = await this.db.getCount(
          `SELECT COUNT(*) AS count FROM escalation_history WHERE task_id = $1 AND rule_id = $2 AND status != 'expired'`,
          [task.id, rule.id]
        );
   ...
   */
  async checkDeadlines(warningOverride, maxPriority) {
    ...
    for (const rule of approachingRules) {
      ...
      const tasks = await this.db.getMany(
        `SELECT * FROM tasks
         WHERE status IN ('todo', 'in_progress', 'unblocked')
    ...
    for (const rule of missedRules) {
      const tasks = await this.db.getMany(
        `SELECT * FROM tasks
         WHERE status IN ('todo', 'in_progress', 'blocked', 'unblocked', 'waiting')
           AND deadline IS NOT NULL
           AND deadline < NOW()
           ${priorityFilter}
         ORDER BY deadline ASC`
   ...
   */
  async processBlockedTasks(maxPriority) {
    const rules = await this.db.getMany(
      ...
      const tasks = await this.db.getMany(
        `SELECT t.* FROM tasks t
         WHERE t.status = 'blocked'
           AND t.updated_at < NOW() - INTERVAL '${threshold} minutes'
           ${agentFilter} ${catFilter} ${priorityFilter}
         ORDER BY t.priority ASC`
      );
  ...
  async checkUrgentTasks() {
    ...
    for (const rule of rules) {
      ...
      const tasks = await this.db.getMany(
        `SELECT t.* FROM tasks t
         WHERE t.priority = 1
           AND t.status IN ('todo', 'in_progress', 'unblocked')
           AND t.updated_at < NOW() - INTERVAL '${threshold} minutes'
  ...
  /**
   * Check if an escalation/nudge target is within working hours.
   * Returns true if the target should be woken, false if off-duty.
   */
  async _isTargetAvailable(targetAgentId, task) {
    // after_hours_auth on the task overrides everything
    if (task?.after_hours_auth) return true;

    if (targetAgentId === 'human') {
      const esc = this.cfg?.escalation || {};
      const humanAgent = {
        working_hours_start: esc.human_working_hours_start || '08:00',
        working_hours_end:   esc.human_working_hours_end || '17:00',
        working_days:        esc.human_working_days || [1, 2, 3, 4, 5],
        timezone:            esc.human_timezone || this.cfg?.timezone || 'America/Toronto',
      };
      return isWithinWorkingHours(humanAgent);
    }

    const agent = await this.db.getOne(
      'SELECT * FROM agent_availability WHERE agent_id = $1', [targetAgentId]
    );
    if (!agent) return true; // unknown agent → allow (safe default)
    if (agent.after_hours_capable) return true;
    return isWithinWorkingHours(agent);
  }
  ...
  async immediateBlockedEscalation(taskId, agentId, reason) {
    ...
    if (!supervisor) {
      // No hierarchy defined — try to fire using 'blocked' escalation rules immediately
      const rules = await this.db.getMany(
        `SELECT * FROM escalation_rules WHERE trigger_condition = 'blocked' AND enabled = TRUE`
      );
      for (const rule of rules) {
        // Skip max_escalations check on immediate fire — only count escalations from the current blocked cycle
        const recentEsc = await this.db.getOne(
          `SELECT * FROM escalation_history
           WHERE task_id = $1 AND rule_id = $2
             AND status IN ('acknowledged', 'resolved')
        ...
        await this._fireEscalation(task, rule, reason);
      }
      return;
    }

    // After-hours suppression: skip if supervisor is off-duty
    if (!await this._isTargetAvailable(supervisor, task)) {
      this.log.info(`[task-system/escalation] task #${taskId} blocked — supervisor ${supervisor} is off-duty, will escalate when available`);
      return;
    }
    ...
    // Wake the supervisor
    if (supervisor === 'human') {
      await this._sendHumanEscalation(
        escRecord.id,
        `Task #${taskId}: "${task.title}" — BLOCKED by ${assignedAgent}: ${reason}`,
        taskId
      );
    } else if (this.runtime?.system?.runCommandWithTimeout) {
  ...
  async _fireEscalation(task, rule, message) {
    // After-hours suppression: skip if target is off-duty
    if (!await this._isTargetAvailable(rule.to_agent, task)) {
      this.log.info(`[task-system/escalation] task #${task.id} — target ${rule.to_agent} is off-duty, skipping escalation`);
      return;
    }
    ...
    } else if (rule.to_agent === 'human') {
      const smsText = smsMessage || message;
      const msgBody = `Task #${task.id}: "${task.title}" [${task.status}] — ${smsText}`;
      await this._sendHumanEscalation(escRecord.id, msgBody, task.id);
    }
  }
  /**
   * Send a human escalation via openclaw message send.
   * Reads channel/account/target from cfg.escalation.
   */
  async _sendHumanEscalation(escalationId, messageText, taskId) {
    const esc = this.cfg?.escalation || {};
    ...
    if (!channel || !target) {
      this.log.error(`[task-system/escalation] human escalation not configured — set escalation.human_escalation_channel and human_escalation_target in settings`);
      return;
    }

    if (!this.runtime?.system?.runCommandWithTimeout) {
      this.log.error(`[task-system/escalation] runtime not available for human escalation`);
      return;
    }
    ...
    try {
      ...
      await this.db.update('escalation_history', { status: 'resolved', response_at: new Date().toISOString() }, 'id = $1', [escalationId]);
      this.log.info(`[task-system/escalation] human escalation sent via ${channel} for task #${taskId}`);
    } catch (err) {
      this.log.error(`[task-system/escalation] failed to send human escalation via ${channel}: ${err.message}`);
    }
  }
}
