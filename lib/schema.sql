...
CREATE INDEX idx_obs_agent ON observations(agent_id, created_at DESC);
CREATE INDEX idx_obs_active ON observations(created_at DESC) WHERE archived_at IS NULL;
CREATE INDEX idx_obs_importance ON observations(importance DESC) WHERE archived_at IS NULL;
...
CREATE INDEX idx_mem_lt_agent ON memory_long_term(agent_id) WHERE superseded_by IS NULL;
...
CREATE INDEX idx_dream_log_agent ON dream_log(agent_id, created_at DESC);
