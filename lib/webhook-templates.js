...
function flattenPayload(obj, prefix = '', result = {}) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (Array.isArray(value)) {
      result[path] = value;           // keep raw array for {{#each}}
      result[`${path}.length`] = value.length;
      value.forEach((item, i) => {
        if (item !== null && typeof item === 'object' && !Array.isArray(item)) {
          flattenPayload(item, `${path}.${i}`, result);
        } else {
          result[`${path}.${i}`] = item;
        }
      });
...
function evaluateOp(actual, op, expected) {
  switch (op) {
    case 'eq':
      // Handle boolean/string mismatch: true=="true", false=="false"
...
/**
 * Render simple {{var}} placeholders (no block helpers).
 * Supports: {{var}}, {{var || "fallback"}}, {{var ? "yes" : "no"}}
 */
function renderSimpleVars(template, vars) {
  if (!template) return '';
...
/**
 * Render a template string with {{variable}} placeholders and {{#each}} blocks.
 * Supports:
 *   - {{var}} - simple replacement
 *   - {{var || "fallback"}} - fallback if var is undefined
 *   - {{var ? "yes" : "no"}} - ternary (based on truthiness of var)
 *   - {{#each path}}...{{/each}} - iterate over arrays
 *     Inside blocks: {{prop}}, {{@index}} (0-based), {{@number}} (1-based)
 */
function renderTemplate(template, vars) {
  if (!template) return '';

  // Process {{#each path}}...{{/each}} blocks first
  template = template.replace(
    /\{\{#each\s+(.+?)\}\}([\s\S]*?)\{\{\/each\}\}/g,
    (match, arrayPath, blockBody) => {
      const arr = getVar(vars, arrayPath.trim());
      ...
      return arr.map((item, index) => {
        ...
        itemVars['@index'] = index;
        itemVars['@number'] = index + 1;
        return renderSimpleVars(blockBody, itemVars);
      }).join('');
    }
  );

  // Then render remaining simple vars
  return renderSimpleVars(template, vars);
}
