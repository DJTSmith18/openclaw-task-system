...
class Scheduler {
  ...
  _getJobSpecs(agentId, checkInterval) {
    const urgentEnabled = this.config.urgentCycleEnabled !== false;
    const urgentIntervalSeconds = this.config.urgentCycleIntervalSeconds || 30;
    ...
    const specs = [
      {
        ...
        payload: {
          kind: 'agentTurn',
          message: `Run the task system scheduler cycle: use the scheduler_run_cycle tool. Report any escalations triggered and any unblocked tasks pending dispatch. Human escalations are sent automatically by the system — no action needed from you for SMS delivery.`,
          timeoutSeconds: 120,
        },
        delivery: { mode: 'none' },
      },
    ...
    // Urgent fast-track cycle — runs every N seconds, priority=1 only
    if (urgentEnabled) {
      specs.push({
        name: 'Task System: Urgent Cycle',
        schedule: { kind: 'every', everyMs: urgentIntervalSeconds * 1000 },
        sessionTarget: 'isolated',
        wakeMode: 'now',
        payload: {
          kind: 'agentTurn',
          message: 'Run urgent-only scheduler cycle: call scheduler_run_cycle with urgent_only=true. This is a fast-track cycle for priority=1 tasks only. Be brief — reply HEARTBEAT_OK if nothing found.',
          timeoutSeconds: 60,
        },
        delivery: { mode: 'none' },
      });
  ...
  async ensureCronJobs() {
    ...
    // Remove stale jobs (disabled memory features or disabled urgent cycle)
    const before = data.jobs.length;
    data.jobs = data.jobs.filter(j => {
      if (j.name.startsWith('Memory:') && !expectedNames.has(j.name)) {
        return false;
      }
      if (j.name === 'Task System: Urgent Cycle' && !expectedNames.has(j.name)) {
        return false;
      }
      return true;
    });
    ...
    if (added > 0 || updated > 0 || removed > 0) {
      ...
      if (removed > 0) this.log.info(`[task-system/scheduler] removed ${removed} stale memory cron jobs`);
    }
  }
}
