...
class Scheduler {
  constructor(opts) {
    ...
    this.escalationConfig = opts.escalationConfig || {};
  ...
  _getJobSpecs(agentId, checkInterval) {
    const humanSession = this.escalationConfig.human_escalation_session || '';
    const smsInstruction = humanSession
      ? `you MUST use the sessions_send tool to send a message to session "${humanSession}" for EACH pending escalation`
      : 'you MUST send an SMS to the human for EACH pending escalation';
    ...
    return [
      {
        ...
        payload: {
          kind: 'agentTurn',
          message: `Run the task system scheduler cycle: use the scheduler_run_cycle tool. Report any escalations triggered and any unblocked tasks pending dispatch. IMPORTANT: If the result includes pending_human_escalations (non-empty array), ${smsInstruction}. Your sessions_send message MUST begin with "Send the following message by SMS:" on the first line, then the actual SMS content on the following lines. This prefix is required so the session delivers it as an outbound SMS. Include the task number, title, blocked reason, and who blocked it. After sending, use escalation_respond with response "resolve" to mark it delivered.`,
          timeoutSeconds: 120,
        },
        delivery: { mode: 'none' },
      },
