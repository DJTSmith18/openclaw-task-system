...
class TaskDispatcher {
  ...
  async _dispatchAgent(agent) {
    ...
    const readyTodo = dispatchableTodo.filter(t => {
      // Recently unblocked tasks bypass cooldown — agent needs immediate notification
      if (t.metadata?.unblocked_at) return true;
    ...
    // Clear unblocked_at after dispatch so we don't re-notify
    for (const task of toStart) {
      if (task.metadata?.unblocked_at) {
        const meta = { ...task.metadata };
        delete meta.unblocked_at;
        await this.db.update('tasks', { metadata: JSON.stringify(meta) }, 'id = $1', [task.id]);
      }
    }
  ...
  async formatDispatchMessage(toStart, toPreempt, activeSet) {
    ...
    if (toStart.length > 0) {
      ...
      for (const t of toStart) {
        ...
        if (t.metadata?.unblocked_at) line += ' [UNBLOCKED — resuming]';
        lines.push(line);
      }
      lines.push('');
