...
class TaskDispatcher {
    ...
    for (const [creatorId, creatorTasks] of Object.entries(byCreator)) {
      ...
      if (creatorId === 'human') {
        ...
        for (const t of creatorTasks) {
          ...
          meta.creator_notified = new Date().toISOString();
          meta.creator_notified_method = sent ? 'sms' : 'skipped_no_channel';
          await this.db.update('tasks', { metadata: JSON.stringify(meta) }, 'id = $1', [t.id]);
        }
        continue;
      }
      ...
      if (!creator) {
        ...
        for (const t of creatorTasks) {
          const meta = t.metadata || {};
          meta.creator_notified = new Date().toISOString();
          meta.creator_notified_skipped = 'agent_not_found';
          await this.db.update('tasks', { metadata: JSON.stringify(meta) }, 'id = $1', [t.id]);
        }
        continue;
      }
      ...
      // Mark all as notified (even on wake failure — retry would spam)
      for (const t of creatorTasks) {
        const meta = t.metadata || {};
        meta.creator_notified = new Date().toISOString();
        await this.db.update('tasks', { metadata: JSON.stringify(meta) }, 'id = $1', [t.id]);
  ...
  async _notifyHumanCompletion(tasks) {
    ...
    for (const t of tasks) {
      let line = `#${t.id} "${t.title}" [${t.status}]`;
      // Get the done note
      try {
        const finalLog = await this.db.getOne(
        ...
        if (finalLog?.notes) {
          // Truncate note for SMS
          const note = finalLog.notes.length > 120 ? finalLog.notes.slice(0, 117) + '...' : finalLog.notes;
          line += ` — ${note}`;
        }
      } catch (e) { /* ignore */ }
      lines.push(line);
