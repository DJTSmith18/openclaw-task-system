...
function wakeAgent(agentId, cycle, message) {
  if (!_runtime?.system?.enqueueSystemEvent || !_runtime?.system?.requestHeartbeatNow) {
    _logger.error('[memory-timer] runtime.system.enqueueSystemEvent/requestHeartbeatNow not available');
    return false;
  }
  try {
    ...
    _runtime.system.enqueueSystemEvent(message, { sessionKey });
    _runtime.system.requestHeartbeatNow({ reason: 'memory-cycle', agentId, sessionKey });
    return true;
  } catch (e) {
...
async function triggerCycle(agentId, cycle) {
  if (!_runtime?.system?.enqueueSystemEvent) {
