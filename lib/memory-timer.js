...
/**
 * Wake an agent for a memory cycle using the channel dispatch path.
 * This is the same mechanism messaging channels (Twilio, VoIP SMS, etc.) use
 * to spawn agent sessions — no heartbeat: true required in agent config.
 */
async function wakeAgent(agentId, cycle, message) {
  if (!_runtime?.channel?.reply?.dispatchReplyFromConfig) {
    _logger.error('[memory-timer] runtime.channel.reply not available — cannot dispatch');
    return false;
  }
  try {
    // Log the cycle attempt IMMEDIATELY so the timer won't re-fire on the next tick.
    // Without this, if the agent session fails/times out without calling logCycle(),
    // the timer would re-fire every 60 seconds indefinitely.
    try {
      await _db.insert('dream_log', {
        agent_id: agentId,
        cycle_type: cycle,
        metadata: JSON.stringify({ status: 'dispatched' }),
      });
    } catch (logErr) {
      _logger.error(`[memory-timer] failed to pre-log cycle ${agentId}:${cycle}: ${logErr.message}`);
    }

    // Isolated session per cycle run — matches openclaw's cron pattern:
    //   agent:<agentId>:memory:<cycle>:run:<uuid>
    // Each run gets a unique UUID so cycles never collide with each other
    // or with the agent's main session.
    const runId = randomUUID();
    const sessionKey = `agent:${agentId}:memory:${cycle}:run:${runId}`;
    _logger.info(`[memory-timer] waking ${agentId} for ${cycle} with session ${sessionKey}`);

    // Build inbound context — same shape as messaging channels
    const ctx = {
      Body: message,
      ...
      ChatType: 'direct',
      Timestamp: Date.now(),
      OriginatingChannel: 'task-system-ui',
    };

    const finalized = _runtime.channel.reply.finalizeInboundContext(ctx);
    const { dispatcher, replyOptions } = _runtime.channel.reply.createReplyDispatcherWithTyping({
      deliver: async () => {}, // no-op — memory cycles work via tool calls, no reply delivery needed
      onError: (e) => _logger.error(`[memory-timer] dispatch error ${agentId}:${cycle}: ${e}`),
    });

    // Fire-and-forget — the dispatch runs the agent asynchronously
    _runtime.channel.reply.withReplyDispatcher({
      dispatcher,
      run: () => _runtime.channel.reply.dispatchReplyFromConfig({
        ctx: finalized,
        cfg: _cfg,
        dispatcher,
        replyOptions,
      }),
    }).catch((e) => _logger.error(`[memory-timer] dispatch failed ${agentId}:${cycle}: ${e.message}`));
    ...
    for (const agent of agents) {
      ...
      if (dreamCfg.enabled !== false) {
        ...
        if (isDailyDue(runAt, lastRun, tz)) {
          ...
          _logger.info(`[memory-timer] triggering dream for ${agentId} (scheduled ${runAt}, last: ${lastRun ? Math.round((now - lastRun) / 60000) + 'm ago' : 'never'})`);
          await wakeAgent(agentId, 'dream', message);
          fired.push(`${agentId}:dream`);
        }
      }
      ...
      for (const cycle of ['rumination', 'sensor_sweep']) {
        ...
        _logger.info(`[memory-timer] triggering ${cycle} for ${agentId} (last: ${lastRun ? Math.round(elapsed / 60000) + 'm ago' : 'never'})`);
        await wakeAgent(agentId, cycle, message);
        fired.push(`${agentId}:${cycle}`);
      }
    }
...
async function triggerCycle(agentId, cycle) {
  if (!_runtime?.channel?.reply?.dispatchReplyFromConfig) {
    const msg = '[memory-timer] triggerCycle: runtime.channel.reply not available (start() may not have been called)';
  ...
  return wakeAgent(agentId, cycle, message);
}
