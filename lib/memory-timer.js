...
function start(db, runtime, logger, eventBus) {
  ...
  const rtKeys = runtime ? Object.keys(runtime) : [];
  const hasSubagent = !!runtime?.subagent;
  const hasRun = !!runtime?.subagent?.run;
  _logger.info(`[memory-timer] started (60s tick) runtime keys=[${rtKeys.join(',')}] subagent=${hasSubagent} run=${hasRun}`);
