...
async function wakeAgent(agentId, message, timeoutSeconds) {
  ...
  // Prefer subagent API (isolated session, no main session lock)
  if (_runtime?.subagent?.run) {
    try {
      ...
      const result = await _runtime.subagent.waitForRun({ runId, timeoutMs: timeout * 1000 });
      if (result.status !== 'ok') {
      ...
      _logger.error(`[memory-timer] failed to wake agent ${agentId} (subagent): ${e.message}`);
      return false;
    }
  }

  // Fallback: CLI spawn (locks main session)
  if (_runtime?.system?.runCommandWithTimeout) {
    try {
      ...
      _logger.error(`[memory-timer] failed to wake agent ${agentId} (cli): ${e.message}`);
      return false;
    }
  }

  _logger.error('[memory-timer] no runtime method available to wake agent');
  return false;
}
...
function start(db, runtime, logger, eventBus) {
  ...
  const mode = runtime?.subagent?.run ? 'subagent' : 'cli';
  _logger.info(`[memory-timer] started (60s tick, wake mode: ${mode})`);
...
async function triggerCycle(agentId, cycle) {
  if (!_runtime?.subagent?.run && !_runtime?.system?.runCommandWithTimeout) {
    const msg = '[memory-timer] triggerCycle: no runtime method available (start() may not have been called)';
