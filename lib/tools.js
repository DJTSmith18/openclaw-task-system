...
function buildTools(db, runtime, logger, eventBus, cfg) {
  ...
  return [
...

    {
      ...
      execute: wrap(async (p) => {
        const urgentOnly = p?.urgent_only || false;
        ...
        // Skip diagnostics in urgent-only mode for speed
        let diagnostics = undefined;
        if (!urgentOnly && cfg?.debug?.scheduler_diagnostics) {
          diagnostics = {
        ...
        const dispatcher = new TaskDispatcher(db, runtime, log, eventBus, cfg?.scheduler, cfg?.escalation);
        const dispatch = await dispatcher.dispatch({ maxPriority });

        const result = { urgent_only: urgentOnly, stuck, deadlines, escalations, after_hours: afterHours, urgent, unacknowledged, unblocked_pending: unblockedTasks, pending_human_escalations: pendingHumanEscalations, dispatch, ran_at: new Date().toLocaleString('en-US', { timeZone: 'America/Toronto' }) };
