  ...
  return [
...

    {
      ...
      parameters: {
        ...
        properties: {
          ...
          status:   { type: 'string', enum: ['todo', 'in_progress', 'blocked', 'done', 'cancelled'], description: 'New status. Use in_progress when starting work, blocked when you need info (never ask in chat), done when complete.' },
      ...
      execute: wrap(async (p) => {
        ...
        // If unblocking, mark for immediate dispatch notification
        if (task.status === 'blocked' && p.status !== 'blocked') {
          const meta = task.metadata || {};
          meta.unblocked_at = new Date().toISOString();
          // Reset dispatch tracking so agent gets re-notified
...

    {
      ...
      execute: wrap(async (p) => {
        ...
        if (isReassignment && (task.status === 'in_progress' || task.status === 'blocked')) {
          fields.status = 'todo';
        }
      ...
      execute: wrap(async (p) => {
        ...
        const overdue = await db.getCount(
          `SELECT COUNT(*) AS count FROM tasks
           WHERE deadline < NOW() AND status IN ('todo', 'in_progress', 'blocked')`
        );
        const unassigned = await db.getCount(
          `SELECT COUNT(*) AS count FROM tasks
           WHERE assigned_to_agent IS NULL AND status IN ('todo', 'in_progress')`
        );
...

    {
      ...
      description: 'Run a full scheduler cycle: stuck task detection, deadline checks, escalation processing, after-hours detection, urgent task monitoring, task dispatch with priority preemption and aging.',
      parameters: {
        type: 'object',
        properties: {},
      },
      execute: wrap(async () => {
        ...
        return ok({ stuck, deadlines, escalations, after_hours: afterHours, urgent, unacknowledged, dispatch, ran_at: new Date().toLocaleString('en-US', { timeZone: 'America/Toronto' }) });
      }),
    },

    {
      ...
      execute: wrap(async () => {
        const openTasks = await db.getCount(
          `SELECT COUNT(*) AS count FROM tasks WHERE status IN ('todo', 'in_progress', 'blocked')`
        );
