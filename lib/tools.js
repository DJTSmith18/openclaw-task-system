...
function buildTools(db, runtime, logger, eventBus, cfg) {
  ...
  return [
...

    {
      ...
      parameters: {
        type: 'object',
        required: ['title', 'created_by_agent', 'assigned_to_agent'],
        properties: {
          title:              { type: 'string', description: 'Task title' },
          description:        { type: 'string', description: 'Detailed description' },
          status:             { type: 'string', enum: ['todo', 'in_progress', 'blocked', 'unblocked', 'waiting'], description: 'Initial status (default: todo)' },
      ...
      execute: wrap(async (p) => {
        // Hierarchy enforcement: tasks can be assigned up or down the chain
        if (p.assigned_to_agent && p.created_by_agent) {
          ...
          if (!isSelf) {
            ...
            const canAssignUp = await isAboveInHierarchy(db, p.assigned_to_agent, p.created_by_agent);
            if (!canAssignDown && !canAssignUp) {
              return err(`Agent "${p.created_by_agent}" cannot create tasks for "${p.assigned_to_agent}" — agents can only assign tasks within their hierarchy chain`);
            }
          }
        }
...

    {
      ...
      parameters: {
        ...
        properties: {
          ...
          metadata:           { type: 'object', description: 'Merged into existing metadata (does not replace). Set a key to null to clear it.' },
        },
      },
      execute: wrap(async (p) => {
        const fields = {};
        ...
        // metadata is merged (not replaced) via JSONB || operator — handled below
        const mergeMetadata = p.metadata !== undefined ? p.metadata : null;
        ...
        if (Object.keys(fields).length === 0 && !mergeMetadata) return err('No fields to update');

        // If metadata merge requested, use JSONB || to merge keys (not replace)
        if (mergeMetadata) {
          const keys = Object.keys(fields);
          const vals = Object.values(fields);
          const setClauses = keys.map((k, i) => `${k} = $${i + 1}`);
          const metaIdx = keys.length + 1;
          const taskIdx = keys.length + 2;
          setClauses.push(`metadata = COALESCE(metadata, '{}'::jsonb) || $${metaIdx}::jsonb`);
          vals.push(JSON.stringify(mergeMetadata));
          vals.push(p.task_id);
          const sql = `UPDATE tasks SET ${setClauses.join(', ')}, updated_at = NOW() WHERE id = $${taskIdx} RETURNING *`;
          const result = await db.query(sql, vals);
          if (result.rows.length === 0) return err(`Task #${p.task_id} not found`);
    ...
    {
      name: 'task_status',
      description: 'Change task status. WORKFLOW: (1) Set "in_progress" IMMEDIATELY when starting. (2) Set "waiting" when you sent a message and are awaiting a response — NO escalation. (3) Set "blocked" only when truly stuck and need supervisor help — triggers escalation. (4) Set "done" when complete. Requires a note. Auto-logs time from in_progress.',
      parameters: {
        type: 'object',
        ...
        properties: {
          ...
          status:   { type: 'string', enum: ['todo', 'in_progress', 'blocked', 'unblocked', 'waiting', 'done', 'cancelled'], description: 'New status. Use in_progress when starting, waiting when awaiting external response (no escalation), blocked when truly stuck (triggers escalation), done when complete.' },
      ...
      execute: wrap(async (p) => {
        ...
        // Mandatory meaningful note for blocked/waiting and unblock transitions
        ...
        if (p.status === 'waiting' && (!p.note || !p.note.trim())) {
          return err('A note is REQUIRED when setting status to waiting. Explain what you are waiting for and from whom.');
        }
        ...
        if (task.status === 'waiting' && p.status !== 'waiting' && (!p.note || !p.note.trim())) {
          return err('A note is REQUIRED when transitioning from waiting. Explain the response received or why the wait ended.');
        }
        ...
        // If leaving waiting, reset dispatch tracking so fresh cycle starts
        if (task.status === 'waiting' && p.status !== 'waiting') {
        ...
        if (task.status === 'blocked' && p.status === 'unblocked') {
          ...
          const { rowCount: expiredCount } = await db.query(
            `UPDATE escalation_history SET status = 'expired'
             WHERE task_id = $1 AND status = 'pending'`,
            [p.task_id]
          );
          if (expiredCount > 0) {
            emit('escalation', { action: 'expired', id: p.task_id, summary: `${expiredCount} escalation(s) expired on unblock` });
          }
        }
...

    {
      ...
      execute: wrap(async (p) => {
        ...
        // Hierarchy enforcement: can assign up or down the chain
        ...
        if (!isSelf) {
          ...
          const canAssignUp = await isAboveInHierarchy(db, p.assigned_to_agent, p.agent_id);
          if (!canAssignDown && !canAssignUp) {
            return err(`Agent "${p.agent_id}" cannot assign tasks to "${p.assigned_to_agent}" — agents can only assign tasks within their hierarchy chain`);
          }
        }
      ...
      execute: wrap(async (p) => {
        ...
        const overdue = await db.getCount(
          `SELECT COUNT(*) AS count FROM tasks
           WHERE deadline < NOW() AND status IN ('todo', 'in_progress', 'blocked', 'unblocked', 'waiting')`
        );
        const unassigned = await db.getCount(
          `SELECT COUNT(*) AS count FROM tasks
           WHERE assigned_to_agent IS NULL AND status IN ('todo', 'in_progress', 'unblocked', 'waiting')`
        );
...

    {
      ...
      description: 'Run a scheduler cycle: stuck task detection, deadline checks, escalation processing, after-hours detection, urgent task monitoring, unblocked task notification, task dispatch with priority preemption and aging. Use urgent_only=true for a lightweight fast-track cycle that only processes priority=1 tasks.',
      parameters: {
        type: 'object',
        properties: {
          urgent_only: {
            type: 'boolean',
            description: 'Only process urgent (priority=1) tasks. Runs a lightweight subset of checks for fast dispatch.',
          },
        },
      },
      execute: wrap(async (p) => {
        const { runCycle } = require('./scheduler-timer');
        const result = await runCycle({
          db, runtime, logger: log, eventBus, cfg,
          urgentOnly: p?.urgent_only || false,
          includeDiagnostics: !p?.urgent_only && cfg?.debug?.scheduler_diagnostics,
        });
        return ok(result);
      }),
    },

    {
      ...
      execute: wrap(async () => {
        const openTasks = await db.getCount(
          `SELECT COUNT(*) AS count FROM tasks WHERE status IN ('todo', 'in_progress', 'blocked', 'unblocked', 'waiting')`
        );
