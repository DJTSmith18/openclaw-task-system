  ...
  return [
...

    {
      ...
      execute: wrap(async (p) => {
        ...
        // Clear nudge metadata on status transitions
        {
          const meta = task.metadata || {};
          let metaDirty = false;
          // in_progress → anything: clear in-progress nudge tracking
          if (task.status === 'in_progress' && p.status !== 'in_progress') {
            delete meta.last_nudge_at; delete meta.nudge_count; metaDirty = true;
          }
          // todo → in_progress: clear unstarted nudge tracking
          if (task.status === 'todo' && p.status === 'in_progress') {
            delete meta.last_unstarted_nudge_at; delete meta.unstarted_nudge_count; metaDirty = true;
          }
          // blocked → anything: clear blocked nudge tracking
          if (task.status === 'blocked' && p.status !== 'blocked') {
            delete meta.last_blocked_nudge_at; delete meta.blocked_nudge_count; metaDirty = true;
          }
          if (metaDirty && !updateData.metadata) {
        ...
        if (task.status === 'blocked' && p.status === 'unblocked') {
          const meta = JSON.parse(updateData.metadata || JSON.stringify(task.metadata || {}));
...

    {
      ...
      description: 'Run a full scheduler cycle: agent nudges, stuck task detection, deadline checks, escalation processing, after-hours detection, urgent task monitoring, unblocked task notification, task dispatch with priority preemption and aging.',
      parameters: {
        type: 'object',
        properties: {},
      },
      execute: wrap(async () => {
        ...
        if (cfg?.debug?.scheduler_diagnostics) {
          diagnostics = {
        ...
        // Nudge agents before escalation checks
        const { NudgeEngine } = require('./nudge-engine');
        const nudger = new NudgeEngine(db, runtime, log, eventBus, cfg);
        const nudges = await nudger.runAll();
        ...
        // Process pending human escalations that were created but never delivered
        // (e.g. immediate escalation fired but couldn't wake the scheduler at that time)
        const pendingHumanEscalations = await db.getMany(
        ...
        const result = { nudges, stuck, deadlines, escalations, after_hours: afterHours, urgent, unacknowledged, unblocked_pending: unblockedTasks, pending_human_escalations: pendingHumanEscalations, dispatch, ran_at: new Date().toLocaleString('en-US', { timeZone: 'America/Toronto' }) };
