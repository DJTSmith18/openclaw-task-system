...
function buildTools(db, runtime, logger, eventBus, cfg) {
  ...
  return [
    ...
    {
      name: 'task_status',
      description: 'Change task status. Requires a note explaining the change. Automatically logs time if moving from in_progress.',
      parameters: {
        type: 'object',
        ...
        properties: {
          ...
          status:   { type: 'string', enum: ['todo', 'in_progress', 'blocked', 'done', 'cancelled'], description: 'New status' },
    ...
    {
      name: 'worklog_add',
      description: 'Add a work log entry for a task: note, time spent, or any action.',
      parameters: {
        type: 'object',
        required: ['task_id', 'agent_id'],
        properties: {
...

    {
      ...
      execute: wrap(async () => {
        ...
        return ok({ stuck, deadlines, escalations, after_hours: afterHours, urgent, dispatch, ran_at: new Date().toLocaleString('en-US', { timeZone: 'America/Toronto' }) });
      }),
    },
