...
const { isWithinWorkingHours } = require('./time-utils');
...
let _normalRunning = false;
let _urgentRunning = false;
...
async function runNudges({ db, runtime, logger, eventBus, cfg }) {
  ...
  // After-hours check: skip nudge if target agent is off-duty (unless task has after_hours_auth)
  async function isAgentOnDuty(agentId, task) {
    if (task.after_hours_auth) return true;
    const agent = await db.getOne('SELECT * FROM agent_availability WHERE agent_id = $1', [agentId]);
    if (!agent) return true; // unknown agent → allow
  ...
  for (const task of stuckInProgress) {
    ...
    if (!await isAgentOnDuty(task.assigned_to_agent, task)) continue;
  ...
  for (const task of blockedTasks) {
    ...
    if (!await isAgentOnDuty(supervisor, task)) continue;
...
async function normalTick() {
  if (!_db) return;
  if (_normalRunning) return; // prevent overlapping ticks
  _normalRunning = true;
...
async function urgentTick() {
  if (!_db) return;
  if (_urgentRunning) return; // prevent overlapping ticks
